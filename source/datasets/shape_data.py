...
class ShapeDataset(Dataset):
    ...
    def _process(self):
        ...
        elif "crossdocked" in self.raw_path:
            all_mols = []
            index_path = os.path.join(self.raw_path, 'index.pkl')
            with open(index_path, 'rb') as f:
                index = pickle.load(f)
            for i, (_, ligand_fn, _) in enumerate(tqdm(index)):
                #tmp = "/"+"/".join(ligand_fn.split("/")[2:])
                #if len(all_mols) > 2000: continue
                rdmol = next(iter(Chem.SDMolSupplier(self.raw_path +ligand_fn, sanitize=False, removeHs=True)))
...
def sample_points_for_pointcloud(mesh, point_clouds, num_samples, loss_type):
    ...
    #other_withinmesh_points = trimesh.sample.volume_mesh(mesh, int(num_samples / 2) - len(withinmesh_points))
    #withinmesh_points = np.concatenate((withinmesh_points, other_withinmesh_points), axis=0)
