...
class PointCloud_AE(nn.Module):
	def __init__(self, config):
		...
		if config.encoder == 'VN_DGCNN':
			self.encoder = VN_DGCNN_Encoder(config.hidden_dim, config.latent_dim, config.enc_layer_num, config.num_k)
		elif config.encoder == 'VN_Resnet':
			self.encoder = VN_Resnet_Encoder(config.hidden_dim, config.latent_dim, config.enc_layer_num, config.num_k)

		self.generator = DecoderInner(config.point_dim, config.latent_dim, config.hidden_dim, config.dec_layer_num, config.loss_type)
...
class VN_DGCNN_ESPShape_Encoder(nn.Module):
	def __init__(self, hidden_dim, latent_dim, layer_num, num_k):
		super(VN_DGCNN_Encoder, self).__init__()
		self.layer_num = layer_num
		self.num_k = num_k
		self.blocks = []
		self.eq_conv_pos = VNLinearLeakyReLU(2, hidden_dim)
		self.inv_conv_esp = nn.Linear(9, hidden_dim * 3, bias=False)
		final_input_dim = 0
		for i in range(layer_num):
			self.blocks.append(
				VNLinearLeakyReLU(2*hidden_dim, hidden_dim),
				nn.Linear(9, hidden_dim * 3, bias=False)
			)
			final_input_dim += hidden_dim
	...
	def forward(self, input):
		...
		"""
		input_pos, input_esp = input
		batch_size = input_pos.size(0)
		try:
			input_pos = input_pos.transpose(2, 3)
		except:
			pdb.set_trace()
		
		feat = get_graph_feature_cross(input_pos, k=self.num_k)
		hidden = self.eq_conv_pos(feat)
		hidden = self.pool(hidden, dim=-1)
		hiddens = []
		
		for i in range(self.layer_num):
			hidden_feat = get_graph_feature_cross(hidden, k=self.num_k)
			hidden = self.blocks[i][0](hidden_feat)
			hidden = self.pool(hidden)
			hiddens.append(hidden)
